...
LIST_HEADER = "[@Sensible_userbot-List] List **{}({})**\n\n"
...
async def removelists(event):
    ...
    if textx:
        x = re.search(r"\[@Sensible_userbot-List] List \*\*(\w*)", textx.text)
        listname = x.group(1)
...
CMD_HELP.update({
    "lists": [
        ...
        "By replying to a @Sensible_userbot List message(identified by "
        "\n'[@Sensible_userbot-List]' in the beginning of a userbot message), "
        "\nyou can omit <listname> from all commands (except $<listname>)."
        "\n@Sensible_userbot will recognize the list from the replied message."
    ]
})
