...
#
""" @Sensible_userbot Exclude, a module for excluding a group from the @Sensible_userbot event handler. """
...
async def exclude_grp(excl):
    ...
    await excl.edit(
        f"`This chat (ID: {chat_id}, Exclude type: {exclude_type}) has been added to @Sensible_userbot Exclude!`"
    )
...
async def unexclude_grp(excl):
    ...
    await excl.edit(
        f"`This chat (ID: {chat_id}) has been removed from @Sensible_userbot Exclude!`"
    )
...
async def listexclude_grp(excl):
    ...
    if resp != "":
        resp = f"`Here is the list of the excluded chats in @Sensible_userbot Exclude:`\n\n{resp}"
    else:
        resp = f"`There are no excluded chats in @Sensible_userbot Exclude.`"
...
CMD_HELP.update({
    "@Sensible_userbot exclude": [
        '@Sensible_userbot Exclude',
        "@Sensible_userbot EXCLUDE IS CURRENTLY IN BETA, and some features MAY NOT WORK PROPERLY. "
        "@Sensible_userbot is not responsible for anything happening because of the bugs.\n\n"
        " - `.exclude [chatid] (in|all)`: Exclude this (or the specified) group from the @Sensible_userbot event handler. "
        "This means that @Sensible_userbot will work restricted in that group, by not responding to incoming "
        "triggers (such as mention while AFK, filters, notes...) and/or your commands (such as .alive).\n"
        " - `.unexclude [chatid]`: Unexclude this (or the specified) group from the @Sensible_userbot event handler. @Sensible_userbot "
        ...
        "able to trigger @Sensible_userbot with notes, filters and mentioning while AFK. Also, AFK module "
        "won't un-AFK you when you send a message.\n"
        " - `all`: This will exclude **all** events, meaning that everything excluded by `in` plus "
        "you won't be able to execute commands. @Sensible_userbot will pretend not to exist on that group when "
        "this option is set.\n\n"
